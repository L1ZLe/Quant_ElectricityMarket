...
def load_models(model_name):
    ...
    if model_name.endswith('.pkl'):
        ...
        raise ValueError('Model name must end with .pkl or .keras')
...
def calculate_metrics(model_name, df):
    ...
    # plot_predictions(model_name, df)
        ...
        for t in range(len(test_data)):
            ...
            pred_and_actual = pd.DataFrame({'Predicted': [np.array(predictions).flatten()[-1]], 'Actual': [test_data.values.flatten()[-1]]})
            if 'pred_and_actual_table' not in st.session_state:
                st.session_state['pred_and_actual_table'] = pred_and_actual
            else:
                st.session_state['pred_and_actual_table'] = pd.concat([st.session_state['pred_and_actual_table'], pred_and_actual.iloc[-1:]], ignore_index=True)
            st.write(st.session_state['pred_and_actual_table'])
